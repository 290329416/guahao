...
class PageModel extends AdminModel {
    ...
    public function delete_page($page_id, $tableName = 'page') {
        ...
        //清memcache缓存
        $this->memcache->delete(MEMPREFIX . 'article:hits' . $page_id);
        $this->memcache->delete(MEMPREFIX . 'category');
        $this->memcache->delete(MEMPREFIX . 'article:detail' . $page_id);

        return $this->db->delete($tableName, array('id' => $page_id));
    }
    ...
    public function page_display($page_detail, $tableName = 'page') {
        ...
        return $this->db->update($tableName, $page_detail, array('id' => $page_detail['id']));
    }
    ...
    public function page_update($page_id, $data, $tableName = 'page') {
        //清memcache缓存
        $this->memcache->delete(MEMPREFIX . 'category');
        $this->memcache->delete(MEMPREFIX . 'page:detail' . $page_id);
        return $this->db->update($tableName, $data, ['id' => $page_id]);
    }
