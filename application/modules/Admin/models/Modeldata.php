<?php if(!defined('APP_PATH')) exit('No direct script access allowed');
	...
	class ModeldataModel extends AdminModel
	{
		...
		public function edit_model($data,$modelid){
			return $this->db->update('model',$data,['modelid'=>$modelid]);
		}
		...
		public function delete_model($modelid){
			return $this->db->delete('model',['modelid'=>$modelid]);
		}
		...
		public function delete_model_fields($modelid){
			return $this->db->delete('model_field',['modelid'=>$modelid]);
		}
		...
		public function drop_model_table($tablename){
			return (boolean)$this->db->query("DROP TABLE IF EXISTS `$tablename`");
		}
		...
		public function enable_model($modelid){
			return $this->db->update('model',['status'=>1,'update_time'=>$_SERVER['time']],['modelid'=>$modelid]);
		}
		...
		public function disable_model($modelid){
			return $this->db->update('model',['status'=>0,'update_time'=>$_SERVER['time']],['modelid'=>$modelid]);
		}
