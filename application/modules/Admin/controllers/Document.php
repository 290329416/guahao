...
class DocumentController extends AdminController {
    ...
    public function indexAction() {
        ...
        //获得栏目总数
        $count = $this->db_category->get_category_count();
        //栏目
        //$data['category'] = $this->db_category->categoryList(NULL, 0, $count, 'orderid');
        $data['category'] = $this->db_category->catelist();
        ...
        foreach($data['data'] as $k=>$v){
            $path[$k] = $this->db_category->get_catpath($v['catid']);
        }
            ...
            if ($continue) {
                ...
                $ret = $this->db_document->addNews(
                        [
                            ...
                            'defaultpic' => $savePath,
                            'en_keywords' => misc::getEnKeywords($keywords),
                        ]
                );
                if ($ret) {
                    Alert::success('添加成功！');
    ...
    public function batchAddAction() {
        ...
        if ($_FILES['news']['name']) {
            ...
            //关键字(如果没写关键字，在标题中取。如果没取到，标题作为关键词)
            $data['keywords'] = empty($this->getRequest()->getPost('keywords', '')) ?
                    implode('   ', misc::getKeywords($data['title'])) :
                    $this->getRequest()->getPost('keywords', '');
            ...
            $ret = $this->db_document->addNews(
                    [
                        ...
                        'url' => '',
                        'description' => mb_substr(strip_tags($data['content']), 0, 78, 'UTF-8'),
                        'inputtime' => $_SERVER['REQUEST_TIME'],
                        'status' => 99,
    ...
    public function deleteDocumentAction() {
        ...
        if ($result) {
            //删除前台缓存
            $memkey = MEMPREFIX . 'article:detail' . $document_id;
            $this->memcache->delete($memkey);
