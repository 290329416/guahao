...
class misc {
    ...
    static function sendmail($username, $email, $subject, $message) {
        $mailconf['sendtype'] = 1;
        //$mailconf['smtplist'][0] = ['email' => 'anhaoadmin@163.com', 'host' => 'smtp.163.com', 'port' => 25, 'user' => 'anhaoadmin', 'pass' => '54kele45'];
     ...
     * @param type $array 必需。规定输入的数组参数。
     * @param type $num 可选。默认是 1。规定返回多少个随机的元素。
     * @return type
     * @author yangguofeng
     */
     ...
     * @return string
     */
    public static function uid($time){
        
        $string = md5(md5(date('Ymd',$time)));
        $str = substr($string,25,5).date('ymd',$time);
        return $str;
